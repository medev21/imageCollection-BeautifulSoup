...
for item in img_url_list:
    ...
    #check if year is between 1938 and 1950, no worldcups existed due to WWII
    if 1938 < year < 1950:
        year -= 4   #decrease by 4 years
        next    #next iteration
    else:
        r = requests.get(item) #get url
        with open(str(year) + "_champion.jpg", 'wb') as outfile:
            outfile.write(r.content) #download file
